...
export const monthsArray = createMonthArray().reverse()
...
export const createdAreaIDCData = uuidList.map((uuid) => {
    const data = []
...
const createArrayWithLast5Years = (): number[] => {
    const years = []
    const currentDate = new Date()
    const currentYear = currentDate.getFullYear()
...
export const yearsArray = createArrayWithLast5Years().reverse()
